...
export function Philosophy() {
  return (
    <div className="flex flex-col items-center gap-10 bg-cream px-page-x py-section-y sm:gap-16 sm:py-section-y-lg lg:gap-24">
      <div className="flex max-w-prose flex-col items-center gap-4 text-center">
        <p className="text-section-label font-medium uppercase text-ink">Our Philosophy</p>
        <h2 className="font-serif text-section-title text-ink">Crafted With Purpose.</h2>
      ...
      <div className="grid w-full max-w-shell grid-cols-1 gap-12 sm:grid-cols-3 sm:gap-8 lg:gap-12">
        {pillars.map(({ number, title, icon: Icon, body }) => (
          <div key={title} className="relative flex flex-col gap-4 overflow-hidden sm:gap-8">
            <div className="flex items-center gap-4">
              ...
              <span className="text-section-label font-medium tracking-[3.3px] text-ink/50">
                {number}
              </span>
            </div>
            <h3 className="font-serif text-section-title text-ink">{title}</h3>
            <p className="text-body font-light text-ink/60">{body}</p>
            <span
              aria-hidden
